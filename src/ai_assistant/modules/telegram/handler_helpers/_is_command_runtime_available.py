...
def _is_command_runtime_available(self, command_name: str) -> bool:
    ...
    if command_name == "mpc":
        return self._mpc_controller is not None or self._remote_ws_hub is not None
    if command_name == "player":
        return (
            self._media_controller is not None
            or self._output_controller is not None
            or self._remote_ws_hub is not None
        )
