...
async def handle_pc_share(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    if await self._reject_if_not_allowed(update):
        return
    if await self._reject_if_no_message_permission(update, "general", "usage"):
        return
    if await self._reject_if_no_message_permission(update, "command", "pc_share"):
        return
    ...
    if len(context.args) < 2:
        await update.effective_message.reply_text("Usage: /pc_share <client_id> <telegram_user_id>")
        return
    ...
    try:
        await self._remote_device_service.share_client(
            owner_user_id=update.effective_user.id,
            client_id=client_id,
            target_user_id=target_user_id,
        )
    except OwnershipError:
        await update.effective_message.reply_text("Only owner can share this client.")
        return
    except RemoteDeviceError as exc:
        await update.effective_message.reply_text(f"Share failed: {exc}")
        return
    ...
    await update.effective_message.reply_text(
        f"Access granted: client={client_id}, user={target_user_id}"
    )
